...
from dgl import function as fn, DGLGraph
...
class GATLayer(torch.nn.Module):
    ...
    def __init__(self, num_in_features, num_out_features, num_of_heads, concat=True, activation=nn.ELU(),
        ...
        self.linear = nn.Linear(num_of_heads * num_out_features, num_out_features)
...
class GATLayerImp3(GATLayer):
    ...
    def __init__(self, num_in_features, num_out_features, num_of_heads, concat=True, activation=nn.ELU(),
        ...
        self.batch_norm = nn.BatchNorm1d(num_out_features * num_of_heads if concat else num_out_features)
    def forward(self, data):
        ...
        #
        gatv2conv = GATv2Conv(self.num_in_features, self.num_out_features, self.num_of_heads)
        #使用batch normalization
        out_nodes_features = self.batch_norm(out_nodes_features.view(-1, out_nodes_features.size(-1))).view_as(
            out_nodes_features)
    ...
    def __init__(self,
        ...
        if isinstance(in_feats, tuple):
        	...
        	if share_weights:
	            ...
	            self.fc_dst = nn.Linear(
	                self._in_src_feats, out_feats * num_heads, bias=bias)
        self.attn = nn.Parameter(nn.FloatTensor(size=(1, num_heads, out_feats)))
        self.feat_drop = nn.Dropout(feat_drop)
        self.attn_drop = nn.Dropout(attn_drop)
        self.leaky_relu = nn.LeakyReLU(negative_slope)
        if residual:
...
class GAT(nn.Module):
    ...
    def __init__(self, d_model, in_feature, num_heads_per_layer, num_features_per_layer,
        ...
        for i in range(num_of_layers):
            ...
            # )
            layer= GAT2Conv(
                    in_feats=num_features_per_layer[i] * num_heads_per_layer[i],
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, x, padding_masks, future_mask=True, output_attentions=False, batch_temporal_mat=None):
        ...
        """
        batch_size, seq_len, d_model = x.shape #(64,128,256)
        ...
        # print("batch_temporal_mat:",batch_temporal_mat)
        if self.add_temporal_bias:
...
#
#     def __init__(self, num_heads, d_model, dim_out, attn_drop=0., proj_drop=0.,
#                  add_cls=True, device=torch.device('cuda'), add_temporal_bias=True,
#                  temporal_bias_dim=64, use_mins_interval=False):
#         super().__init__()
#         assert d_model % num_heads == 0
#
...
#         # 处理填充掩码
#         if padding_masks is not None:
#             # padding_masks 已经是 (B, 1, T, T)，我们只需要将第二个维度从1扩展到self.num_heads
#             padding_mask = padding_masks.expand(batch_size, self.num_heads, seq_len, seq_len)
...
#                 # 处理未来掩码
#         if future_mask:
#             future_mask = torch.ones((seq_len, seq_len), dtype=torch.bool, device=self.device).triu(
#                 seq_len - seq_len + 1)
#             if mask is not None:
...
#                 batch_temporal_mat = batch_temporal_mat * self.temporal_mat_bias.expand((1, seq_len, seq_len))
#             batch_temporal_mat = batch_temporal_mat.unsqueeze(1)  # (B, 1, T, T)
#             temporal_bias = rearrange(batch_temporal_mat, 'b i j -> 1 b i j').expand(self.num_heads, -1, -1,
#                                                                                      -1).permute(1, 0, 2,
#                                                                                                  3)
#             # 将时间偏差添加到注意力分数中，这里我们将其视为掩码的一部分，通过将其转换为负无穷大来实现
#             # 注意：这种方法可能不是最优的，因为它改变了掩码的意义。更好的方法可能是直接在注意力计算中添加偏差。
#             mask_value = -torch.finfo(query.dtype).max
#             temporal_bias_mask = (temporal_bias == 0).float() * mask_value
#             if mask is not None:
#                 mask = mask.float() + temporal_bias_mask
#                 mask = mask.bool()
#             else:
#                 mask = temporal_bias_mask.bool()
#
#         # 使用pytorch的flash attention
#
#         out = F.scaled_dot_product_attention(
#             query, key, value,
#             attn_mask=mask,
#             dropout_p=self.dropout if self.training else 0.,
#             is_causal=False  # 因果掩码已由future_mask处理
#         )
#         # 4. 解包输出，只保留value部分，并重新排列维度以匹配原始实现
#         out = out.reshape(batch_size, seq_len, 3, self.num_heads, self.d_k)  # (B, T, 3, head, d_k)
#         value_out = out[:, :, 2, :, :]  # 提取value部分
#         value_out = value_out.transpose(1, 2).contiguous().view(batch_size, -1,
#                                                                 self.num_heads * self.d_k)  # (B, T, d_model)
#         # 5. 应用最终的线性投影和dropout
#         out = self.proj(value_out)
#         out = self.proj_drop(out)
#         if output_attentions:
#             # 由于Flash Attention不直接返回注意力分数，这里返回一个占位符
#             return out, None
#         else:
#             return out, None
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, d_model, attn_heads, feed_forward_hidden, drop_path,
        ...
        # self.attention = colMultiHeadedAttention(num_heads=attn_heads, d_model=d_model, dim_out=d_model,
        ...
        self.attention=CollaborativeAttention(
            dim_input=d_model,
            dim_value_all=d_model,
            dim_key_query_all=256,
            dim_output=d_model,
            num_attention_heads=attn_heads,
            attention_probs_dropout_prob=attn_drop,
            use_dense_layer=True,
            use_layer_norm=True,
            mixing_initialization=mixing_initialization,
...
class MaskedLanguageModel(nn.Module):
    ...
    def __init__(self, hidden, vocab_size):
        """
        Args:
            hidden_size: Output size of the BERT model (or any other encoder)
            vocab_size: Total vocabulary size
            config: Optional configuration object, which can contain parameters like layer norm epsilon, bias initialization, etc.
                    Default is None, meaning default values will be used.
            activation_fn: Optional activation function to apply before the linear layer.
                    Default is None, meaning no activation will be applied.
        """
        super().__init__()

        # Default configuration if not provided

        layer_norm_eps = 1e-12  # Example default value
        activation_fn = nn.GELU
        self.layer_norm = nn.LayerNorm(hidden, eps=layer_norm_eps)
        self.dense = nn.Linear(hidden, hidden) if activation_fn is not None else nn.Identity()
        self.activation_fn = activation_fn if isinstance(activation_fn, type) and issubclass(activation_fn,
                                                                                             torch.nn.Module) else nn.Identity()
    ...
    def forward(self, x):
        if self.layer_norm is not None:
            x = self.layer_norm(x)
        if not isinstance(self.dense, nn.Identity):
            x = self.dense(x)
            x = self.activation_fn(x)
        logits = torch.matmul(x, self.classifier.weight.t()) + self.bias
