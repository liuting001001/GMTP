...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, x, padding_masks, future_mask=True, output_attentions=False, batch_temporal_mat=None,chunk_size=512):
        """
        ...
        # 新加内容
        # 分割query, key, value为小块
        num_chunks = (seq_len + chunk_size - 1) // chunk_size
        out = torch.zeros(batch_size, seq_len, self.num_heads * self.d_k, device=x.device)
        #初始化存储先前片段的注意力分数和输出的字典
        prev_attn_chunks = {}
        prev_out_chunks = {}
        for i in range(num_chunks):
            ...
            # 检查是否有先前片段的注意力分数可以重用
            if i > 0:
                prev_attn_chunk = prev_attn_chunks.get((i - 1), None)
                if prev_attn_chunk is not None:
                    # 如果有，则重用先前片段的注意力分数，并跳过当前片段的注意力计算
                    attn_chunk = prev_attn_chunk
                else:
                    # 如果没有，则计算当前片段的注意力分数
                    scores_chunk = torch.matmul(query_chunk, key_chunk.transpose(-2, -1)) * self.scale
                    attn_chunk = F.softmax(scores_chunk, dim=-1)
                    attn_chunk = self.dropout(attn_chunk)
                    prev_attn_chunks[i] = attn_chunk  # 存储当前片段的注意力分数以供后续重用
            else:
                # 对于第一个片段，始终计算注意力分数
                scores_chunk = torch.matmul(query_chunk, key_chunk.transpose(-2, -1)) * self.scale
                # Softmax归一化当前块
                attn_chunk = F.softmax(scores_chunk, dim=-1)
                attn_chunk = self.dropout(attn_chunk)
                prev_attn_chunks[i] = attn_chunk  # 存储第一个片段的注意力分数
            ...
            #新加内容开始
            # 计算当前片段的输出，并存储以供后续可能的重用
            out_chunk = torch.matmul(attn_chunk, value_chunk.transpose(-2, -1)).transpose(-2, -1)
            out[:, start_idx:end_idx, :] = out_chunk.view(batch_size, -1, self.num_heads * self.d_k)
            prev_out_chunks[i] = out_chunk  # 存储当前片段的输出
            # 新加内容结束
